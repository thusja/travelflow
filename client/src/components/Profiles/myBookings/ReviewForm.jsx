...
const ReviewForm = () => {
  ...
  const submitReviewMutation = useMutation({
    mutationFn: (formData) =>
      requestApi(
        ...
        { requireAuth: true, errorMessage: "리뷰 제출 실패" },
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.reviews.reviewable() });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("후기가 성공적으로 제출되었습니다.");
      navigate("/myBookings/review");
    } catch (err) {
      console.error(err);
      toast.error(err.message || "후기 제출 중 오류가 발생했습니다.");
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow mt-30">
      ...
      <div className="mb-4 flex justify-between items-start gap-6">
        {/* 별점 */}
        <div>
          <p className="mb-2 text-gray-700 text-left">별점 선택</p>
      ...
      >
        {submitReviewMutation.isPending ? "제출 중..." : "후기 제출"}
      </button>
    </div>
  );
};
